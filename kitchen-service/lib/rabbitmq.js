...
async function connectToRabbitMQ(orderReceived) {
    ...
    rabbit.on('connection', () => {
        ...
        rabbit.queueDeclare({ queue: fridgeQueue, durable: true });
        connectionIsOk = true;
    })
...
async function processMessageQueue() {
    if (connectionIsOk) {
        ...
        while (messageQueue.length > 0) {
            ...
            await pub.send(fridgeQueue, message);
        }
    }
    else {
